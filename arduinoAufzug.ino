...
void loop()
{
	...
	validInput = sensors_valid(sensorStartP, floorCount) 
		&& sensors_valid(requestStartP, floorCount);
	if ((active && locStop == locNow) || !validInput || digitalRead(sensorDoor))
	{
		digitalWrite(powerP, LOW);
		digitalWrite(powerP + 1, LOW);
    	active = false;
	}
	else if (!active && locStop != locNow && validInput && !digitalRead(sensorDoor))
	{
...
bool sensors_valid(int start, int floors)
{
	int activated = 0;
